Deno.readTextFile('./inputs/day_06_input').then((txt: string) => {
    ...
    const originalMap: LabMap2 = arrays.map((row, i) => {
        return row.map((cell, j) => {
            ...
            if(guardFacing) {
                ...
                return { type: 'VACANT', checkedIn: new Set<Direction2>().add(guardFacing) }
            };
    ...
    for(let i = 0; i <= originalMap.length - 1; i++) {
        for(let j = 0; j <= originalMap[0].length - 1; j++) {
            ...
            if(map[i][j].type === 'VACANT' && !(i === guardPoint[1] && j === guardPoint[2])) {
...
type LabCell2 = { type: 'OBSTACLE' } | { type: 'VACANT', checkedIn: Set<Direction2> } 
