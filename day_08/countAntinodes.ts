...
function calcAntionodes(
  ...
  locations.forEach((loc, i) => {
    locations.forEach((l, j) => {
      if (i === j) return;
      calcAntinodePair(loc, l, width, height).forEach((anti) =>
        output.add(`${anti.i},${anti.j}`)
      );
    });
  });
  return [...output.values()].map((v) => {
...
function calcAntinodePair(
  ...
  return output.filter((o) =>
    o.i >= 0 && o.i < height && o.j >= 0 && o.j < width
  );
}
