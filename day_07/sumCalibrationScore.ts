...
function plusOrMultiple(a: number | undefined, b: number): number[] {
    if(a === undefined) return [b];
    return [a+b, a*b];
}

function reachableNumbers(acc: (number | undefined)[], rest: number[]): number[] {
    if(rest.length === 0) return acc.flatMap((x) => x === undefined ? [] : [x]);
    const next = acc.flatMap((x) => plusOrMultiple(x, rest[0]));
